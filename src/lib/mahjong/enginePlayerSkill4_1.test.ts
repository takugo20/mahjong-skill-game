...
function createTile(
  ...
  return {
    id: `engine-player-skill-4-1-${serialNumber}`,
    suit,
    rank,
    red: false
  };
...
describe("プレイヤースキル4-1のエンジン統合", () => {
  it("通常ツモで手牌と同色の隣接数牌へ1.5倍を適用する", () => {
    const normalTile = createTile(
      "pin",
      1
    );
    const adjacentTile = createTile(
      "man",
      6
    );
