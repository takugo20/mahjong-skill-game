...
describe("4-21のエンジン統合", () => {
  it("発動でMPを消費し予約を追加するが現在の手牌は変えない", () => {
    const state = createState();
    expect(canActivatePlayerSkill4_21(state)).toBe(true);

    const result = activatePlayerSkill4_21(state);
    expect(result.playerMp).toBe(570);
    ...
    expect(canActivatePlayerSkill4_21(result)).toBe(false);
    expect(activatePlayerSkill4_21(result)).toBe(result);
  ...
  it("CPU手番・打牌選択以外・MP不足では発動しない", () => {
    ...
    for (const current of cases) {
      expect(canActivatePlayerSkill4_21(current)).toBe(false);
      expect(activatePlayerSkill4_21(current)).toBe(current);
    }
  });

  it("次局配牌で対子を保証し予約を消費して136枚を維持する", () => {
    const activated = activatePlayerSkill4_21(createState());
    ...
    expect([
      ...
      (effect) => effect.sourceId === "player-skill:4-21"
    )).toBe(false);
