...
function createTile(
  ...
  return {
    id: `engine-player-skill-4-9-${serialNumber}`,
    suit,
    rank,
    red: false
  };
...
describe("プレイヤースキル4-9のエンジン統合", () => {
  it("通常ツモで字牌候補へ2倍を適用する", () => {
    const numberTile = createTile("man", 5);
    const honorTile = createTile("honor", 1);
    const state = prepareDrawState(
      [],
      [numberTile, honorTile],
      [{ id: "4-9", level: 5 }]
    );
  ...
  it("数牌候補だけなら抽選結果を変更しない", () => {
    const firstTile = createTile("pin", 2);
    const secondTile = createTile("sou", 8);
    const state = prepareDrawState(
      [],
  ...
  it("4-2と4-9の両方に該当する字牌へ倍率を乗算する", () => {
    const numberTile = createTile("man", 5);
    const honorTile = createTile("honor", 7);
    const state = prepareDrawState(
      [createTile("honor", 7)],
      [numberTile, honorTile],
      [
