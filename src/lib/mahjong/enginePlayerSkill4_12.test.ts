...
function createTile(
  ...
  return {
    id: `engine-player-skill-4-12-${serialNumber}`,
    suit,
    rank,
    red: false
  };
...
describe("プレイヤースキル4-12のエンジン統合", () => {
  it("通常ツモで萬子候補へ2倍を適用する", () => {
    const otherTile = createTile("sou", 5);
  ...
  it("4-2と4-12の両方に該当する萬子へ倍率を乗算する", () => {
    const otherTile = createTile("sou", 5);
    const manTile = createTile("man", 7);
    const state = prepareDrawState(
      [createTile("man", 7)],
      [otherTile, manTile],
      [
