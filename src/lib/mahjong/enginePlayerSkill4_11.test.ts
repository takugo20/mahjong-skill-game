...
function createTile(
  ...
  return {
    id: `engine-player-skill-4-11-${serialNumber}`,
    suit,
    rank,
    red: false
  };
...
describe("プレイヤースキル4-11のエンジン統合", () => {
  it("通常ツモで筒子候補へ2倍を適用する", () => {
    const otherTile = createTile("man", 5);
    const pinTile = createTile("pin", 1);
  ...
  it("筒子以外の候補だけなら抽選結果を変更しない", () => {
    const firstTile = createTile("sou", 2);
  ...
  it("4-2と4-11の両方に該当する筒子へ倍率を乗算する", () => {
    const otherTile = createTile("man", 5);
    const pinTile = createTile("pin", 7);
    const state = prepareDrawState(
      [createTile("pin", 7)],
      [otherTile, pinTile],
      [
