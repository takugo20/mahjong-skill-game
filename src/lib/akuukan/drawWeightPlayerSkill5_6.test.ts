...
function input() {
  ...
  return {
    ...
    penchanKanchanWinningTileIds: ["tanki"],
    random: vi.fn(() => 0.9)
  };
}

describe("5-6 辺張・嵌張和了牌の重量抽選", () => {
  it("Lv.5では通常牌と辺張・嵌張和了牌の重量比が1対3.5になる", () => {
    const value = input();
  ...
  it("辺張・嵌張和了牌の情報がなければ補正しない", () => {
    const value = input();

    expect(
      drawIndex({
        ...value,
        penchanKanchanWinningTileIds: undefined
      })
    ).toBe(0);
