...
export interface ApplyPlayerSkill4_21AtDealInput {
...
export interface PlayerSkill4_21DealResult {
  readonly akuukan: AkuukanGameState;
...
export function applyPlayerSkill4_21AtDeal(
  input: ApplyPlayerSkill4_21AtDealInput
): PlayerSkill4_21DealResult {
  if (
  ...
  const enabled =
    getEquippedPlayerSkill(input.akuukan, "4-21") !== null &&
    !isAkuukanSourceDisabled(input.akuukan, "player-skill:4-21");

  if (enabled) {
    for (const _reservation of reservations) {
      if (
        reservedTiles.length + 2 > input.remainingHandTileCount
      ) {
        break;
      }
  ...
  return {
    ...
    guaranteedPairCount: reservedTiles.length / 2
  };
}
